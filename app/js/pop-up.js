...
window.addEventListener('keydown', function (event) {
    if (event.keyCode === 27) {
        ...
        if (
         ...
         if (
            popupSuccess.classList.contains('pop-up-success--active')) {
            popupSuccess.classList.remove('pop-up-success--active');

        }
         if (
            popupFail.classList.contains('pop-up-fail--active')) {
...
if (formList) {formList.addEventListener('submit', function validateForm() {
...
if (closeWindow) {closeWindow.addEventListener('click', function (event) {
    event.preventDefault();
